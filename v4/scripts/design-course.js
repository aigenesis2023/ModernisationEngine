 ...
 *
 * Usage:
 *   node v3/scripts/design-course.js --manual          # Write prompt, wait for paste
 *   ANTHROPIC_API_KEY=sk-... node v3/scripts/design-course.js  # Call API directly
 *   node v3/scripts/design-course.js --load response.json      # Load a saved response
 */
...
// ─── Paths ───────────────────────────────────────────────────────────
const PROMPT_PATH = path.resolve('v3/prompts/layout-engine.md');
const COMPONENT_LIB_PATH = path.resolve('v3/schemas/component-library.json');
const CONTENT_PATH = path.resolve('v3/output/content-bucket.json');
const BRAND_PATH = path.resolve('v3/output/brand-profile.json');
const SCHEMA_PATH = path.resolve('v3/schemas/course-layout.schema.json');
const OUTPUT_PATH = path.resolve('v3/output/course-layout.json');
const ASSEMBLED_PROMPT_PATH = path.resolve('v3/output/design-prompt.txt');
...
async function readManualResponse() {
  ...
  console.log('     a. Save it to a file and run: node v3/scripts/design-course.js --load <file>');
...
async function main() {
  ...
  try {
    ...
    const debugPath = path.resolve('v3/output/design-response-raw.txt');
  ...
  console.log('\nNext steps:');
  console.log('  node v3/scripts/generate-images.js    # Generate images');
  console.log('  node v3/scripts/build-course.js        # Build final HTML');
  console.log('\nDone.');
}
