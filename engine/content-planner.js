...
window.ContentPlanner = (function () {
  ...
  function groupIntoSections(slides) {
    ...
    for (var i = 0; i < contentSlides.length; i++) {
      ...
      // This handles courses with no scene boundaries — prevents one giant section.
      if (currentSection && slide.type === 'content') {
        var prevSlide = currentSection.slides[currentSection.slides.length - 1];
        // After 5+ slides, break on topic shift (different title keywords)
        if (currentSection.slides.length >= 5 && prevSlide && isTopicShift(prevSlide.title, slide.title)) {
          shouldBreak = true;
        }
        // After 10+ content slides in one section, force a break if the slide
        // has its own title (even if topics overlap). No e-learning section
        // should have more than ~10 slides — it becomes a wall of content.
        // This is universal: long single-scene courses need logical breaks.
        if (currentSection.slides.length >= 10 && currentSection.type === 'content' &&
            slide.title && !isAutogeneratedLabel(slide.title)) {
          shouldBreak = true;
        }
      }
  ...
  function planSlide(slide) {
    ...
    //
    // IMPORTANT: This dedup is PER-SLIDE only, NOT cross-section.
    // Branching courses intentionally repeat content in different paths
    // (e.g., Non-technical and Technical paths may share quiz questions).
    // The user only sees ONE path, so cross-section "duplicates" are
    // intentional and MUST be preserved. Never add global dedup.
