#!/usr/bin/env node
/**
 * V5 Course Builder — Archetype Recipe Build
 *
 * Reads design-tokens.json (generated by generate-design-tokens.js)
 * and fills components with content from course-layout.json.
 * Visual design comes from archetype recipes in visual-archetypes.json —
 * visual design is deterministic from MD3 palette + archetype recipes.
 *
 ...
 *   3. Typography: text-display/h2/h3/h4/body-lg/body/label-text/blockquote/stat
 *
...
// Each reads visual properties from AR (archetype recipe).
//
...
//     (scale from buildTailwindCSS() → theme-template.css)
//
// Visual contract (from AR — archetype-driven, same for each archetype family):
//   - Shadows, hovers, transitions, gradients, rings
//   - Card backgrounds, border colours (glass vs flat vs solid)
//   - Button styles, input styles
//   - Consistent border-radius and surface rhythm per archetype
// ═══════════════════════════════════════════════════════════════════════
...
function fillHero(comp, variant) {
  ...
  const btn2Bg = 'border border-outline-variant';
  const btn2Round = h.btn1Round || AR.borderRadius?.button || 'rounded-xl';
  const btn2Visual = 'hover:bg-surface-variant transition-colors';
...
function fillAccordion(comp, variant, maxW) {
  ...
  if (variant === 'accent-border') {
    // accent-border variant always uses a visible left accent
...
function fillMCQ(comp, variant, maxW) {
  ...
  // Card styling from archetype recipe
  const cardClass = mc(
    m.cardBg || AR.surface?.card || 'glass-card',
    m.cardPad || 'p-6 md:p-8',
...
<span class="w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold transition-all ${m.letterBg || 'border-2 border-outline-variant/40 group-hover:border-secondary group-hover:bg-secondary/10'} ${m.letterText || 'text-on-surface-variant group-hover:text-secondary'}">${optionLetters[i] || ''}</span>
<span class="text-on-surface font-medium leading-snug" data-edit-path="_items.${i}.title">${esc(item.text || item.title || '')}</span>
</button>`;
    }).join('\n');
...
<div class="mt-8 hidden" data-quiz-feedback></div>
  ...
  const newChoices = items.map((item, i) => {
    return `<button class="${mc(
      'w-full text-left flex items-center gap-5 p-5 md:p-6',
      m.choiceRound || AR.borderRadius?.button || 'rounded-xl',
      m.choiceBg || 'bg-surface-container/60 border border-outline-variant/20',
      m.choiceHover || 'hover:bg-surface-container hover:border-secondary/50 transition-all',
      'cursor-pointer group'
    )}" data-choice="${i}">
<span class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all flex-shrink-0 ${m.letterBg || 'border-2 border-outline-variant/40 group-hover:border-secondary group-hover:bg-secondary/10'} ${m.letterText || 'text-on-surface-variant group-hover:text-secondary'}">${optionLetters[i] || ''}</span>
<span class="text-on-surface leading-relaxed" data-edit-path="_items.${i}.title">${esc(item.text || item.title || '')}</span>
</button>`;
  }).join('\n');
...
// Covers MD3 semantic pairs and inverse patterns.
function bentoCardTextClass(bgClass) {
  ...
  // Inverse: on-background/on-surface used as a dark card bg — pair with surface colour
...
function fillTextInput(comp, maxW) {
  ...
  const cardClass = `${AR.surface?.card || 'glass-card'} p-6 md:p-12 ${AR.borderRadius?.cardLarge || 'rounded-3xl'}`;
...
function fillPathSelector(comp, maxW) {
  ...
  const newButtons = items.map((item, i) =>
    `<button class="group p-6 md:p-8 ${btnBg} ${btnRound} text-left ${btnVisuals}" data-path-option data-path-variable="${esc(item.variable || '')}">
...
function fillBranching(comp, variant, maxW) {
  ...
  const brCfg2 = AR.branching || {};
  const secClass = sectionOnly('py-16');

  const btnBg = brCfg2.buttonBg || AR.surface?.card || 'glass-card border border-outline-variant/20';
  const btnRound = AR.borderRadius?.cardLarge || 'rounded-2xl';
  const btnVisuals = brCfg2.buttonHover || 'hover:border-primary/40 hover:bg-surface-container transition-all duration-200';
  const hasArrow = brCfg2.hasArrow || false;
  const arrowClass = 'mt-4 inline-flex items-center gap-2 text-primary font-bold group-hover:translate-x-2 transition-transform';
...
${altSteps}
  ...
  if (tlCfg.hasNumberedCircles && tlCfg.circleClass) {
    const connectorClass = tlCfg.connectorClass || 'w-0.5 h-full bg-outline-variant/20 mt-4';
    const newSteps = items.map((item, i) => {
      const num = i + 1;
      const isLast = i === items.length - 1;
      return `<div class="flex gap-8">
<div class="flex flex-col items-center">
<div class="${tlCfg.circleClass} flex items-center justify-center font-bold">${num}</div>
...
${newSteps}
  ...
  // Border-l dot layout
  const activeDotClass = tlCfg.activeDotClass || 'absolute -left-[11px] top-0 w-5 h-5 rounded-full bg-primary shadow-[0_0_10px_rgba(100,100,255,0.4)]';
  const inactiveDotClass = tlCfg.inactiveDotClass || 'absolute -left-[11px] top-0 w-5 h-5 rounded-full bg-outline-variant/60';
...
function fillComparison(comp, variant, maxW) {
  ...
  if (variant === 'stacked-rows' && columns.length === 2) {
    const stackedRows = rows.map((row, ri) => {
      ...
      return `<div class="${AR.surface?.card || 'glass-card'} ${AR.borderRadius?.button || 'rounded-xl'} px-5 py-3 flex flex-col md:flex-row items-center gap-2 md:gap-4">
...
function fillStatCallout(comp, variant, maxW) {
  const items = comp._items || [];
  if (items.length === 0) return '';
  const statCfg = AR.stat || {};
  const secClass = sectionOnly('py-16 bg-surface-container-low');

  // Use archetype-driven stat color
  ...
  if (variant === 'card-row') {
    ...
    const cardStats = items.map((item, i) => {
      ...
      return `<div class="${statCardBg} ${statCardRound} p-6 md:p-8 min-w-[200px] flex flex-col">
<div class="text-stat font-headline font-extrabold ${numColor} mb-2" data-counter data-edit-path="_items.${i}.value" data-stat-prefix="${esc(item.prefix||'')}" data-stat-suffix="${esc(item.suffix||'')}">${esc(displayValue)}</div>
...
${cardStats}
  ...
  const newStats = items.map((item, i) => {
    const numColor = unifiedNumColor;
    const displayValue = (item.prefix || '') + (item.stat || item.value || '') + (item.suffix || '');
    return `<div class="${mc('p-8', statCardRound, statCardBg, 'min-w-[120px]')}">
<div class="text-stat font-headline font-extrabold ${numColor} mb-3" data-counter data-edit-path="_items.${i}.value" data-stat-prefix="${esc(item.prefix||'')}" data-stat-suffix="${esc(item.suffix||'')}">${esc(displayValue)}</div>
...
function fillPullquote(comp, variant, maxW) {
  ...
  const bqStyle = pqCfg.blockquoteStyle || 'text-blockquote font-headline';
  const citeStyle = pqCfg.citeClass || 'text-on-surface-variant';
...
${pqCfg.hasDecorativeQuote ? `<span class="${pqCfg.quoteIconClass || 'text-primary/20 text-6xl mb-8'} material-symbols-outlined select-none" aria-hidden="true">${pqCfg.quoteIcon || 'format_quote'}</span>` : '<span class="text-primary/10 text-[6rem] font-serif leading-none select-none pointer-events-none" aria-hidden="true">&ldquo;</span>'}
</div>
...
function fillChecklist(comp, variant, maxW) {
  ...
  const cardClass = mc(clCfg.cardBg || AR.surface?.card || 'glass-card', 'p-6 md:p-8', clCfg.cardRound || AR.borderRadius?.cardLarge || 'rounded-3xl');
  const inputClass = clCfg.inputClass || 'w-7 h-7 rounded border-outline-variant text-secondary focus:ring-secondary bg-transparent cursor-pointer';
  const labelHover = clCfg.labelHover || 'hover:bg-surface-variant/50 transition-colors';
  const spanHover = '';
...
function fillTabs(comp, variant, maxW) {
  ...
  const title = esc(comp.displayTitle || '');
  const secClass = sectionOnly('py-16 bg-surface-container-low');
  // Build tab button classes from archetype recipe
...
function fillFlashcard(comp, variant, maxW) {
  ...
  const fcRound = fcCfg.frontRound || AR.borderRadius?.cardLarge || 'rounded-3xl';
  const backRound = fcCfg.backRound || fcRound;
  ...
  const makeCard = (item, i, large) => {
    ...
    const frontFaceClass = mc(fcCfg.frontBg || AR.surface?.card || 'glass-card', fcRound, 'border border-outline-variant/10', large ? 'p-10 md:p-14' : 'p-6 md:p-8');
    const backFaceClass = mc(fcCfg.backBg || 'bg-secondary-container', backRound, large ? 'p-8 md:p-12 text-center' : 'p-4 md:p-5 text-center');
...
<p class="${fcCfg.backText || 'text-on-primary-container'} font-medium text-${large ? 'base' : 'xs'} leading-snug" data-edit-path="_items.${i}.back">${backText}</p>
</div>
</div>
</div>`;
  };
...
function fillNarrative(comp, variant, maxW) {
  ...
  const title = esc(comp.displayTitle || '');
  const body = comp.body || '';
  // Cap narrative py to py-16 — excessive spacing creates a dead zone above the carousel
  const secClass = sectionOnly('py-16')
...
${listItems}
  ...
  const ktCfg2 = AR.keyTerm || {};
  const cols = items.length <= 2 ? items.length : items.length === 4 ? 2 : 3;
  const newCards = items.map((item, i) =>
    `<div class="${ktCfg2.cardBg || AR.surface?.card || 'glass-card'} p-6 md:p-8 ${ktCfg2.cardRound || AR.borderRadius?.card || 'rounded-2xl'} overflow-hidden ${ktCfg2.accentBorder || 'border-l-4 border-primary'}">
